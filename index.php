...
<body class = "alexa-body">
  ...
  <script>
    jQuery(document).ready(function ($) {
      ...
      const ad_main_select = (prev_box_id, box_id, main_idx) =>({type: AD_MAIN_SELECT,prev_box_id: prev_box_id, main_idx: main_idx,
              ...
              state.boxes.push(
                {
                  id: box_id, 
                  action: 'ad',
                  active:{ main:action.main_idx, sub:0},
                  cases:[ ],
      ...
      $(document).on('click', '.ad.main', function(){
        if( $('#ad-dropdown').css('display') == 'none'){
          ...
          $.each(cases, function(index, item){
            active.push(Number(item.id.split("_")[0]));
          })

          $.each(items, function (index, item){
            if(active.includes(index)){
              $('.ad-new-btn').before('<div class="ad-item" idx="' + items.indexOf(item) + '">' +
                                      '<input readonly="readonly" class="isflow" value="' + item.main + '">' + 
                                    '</div>');
            }else{
              $('.ad-new-btn').before('<div class="ad-item" idx="' + items.indexOf(item) + '">' +
      ...
      $(document).on('click', '.ad.sub', function(){
        if( $('#sub-ad-dropdown').css('display') == 'none'){
          ...
          let active = [];
          $.each(cases, function(index, item){
            if(item.id.startsWith(main_idx)){
              active.push(Number(item.id.split("_")[1]));
            }
          })
      ...
      function showDialog(){
        ...
        while(box_id){
          ...
          if(action == 'hs'){
            ...
            createRow(row_id, box_id, box, action, text);
            let cases = box.cases;
            let active_case = box.active.main + '_' + box.active.sub;
            box_id = 0;
            $.each(cases, function (index, item){
              if(item.id == active_case){
                  box_id = item.next;
              }
            });
      ...
      function createRow(row_id, box_id, box, action, text){
        ...
        switch(action){
          ...
          case 'ad':
            ...
            let maintext = ad[box.active.main].main;
            let subtext;
            if(ad[box.active.main].sub.length){
              subtext = ad[box.active.main].sub[box.active.sub];
            }else{
              subtext = '';
            }
